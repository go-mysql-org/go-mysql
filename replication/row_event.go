...
func (e *RowsEvent) parseFracTime(t Time) interface{} {
	if !e.parseTime {
...
func (e *RowsEvent) decodeValue(data []byte, tp byte, meta uint16) (v interface{}, n int, err error) {
	...
	case MYSQL_TYPE_TIMESTAMP:
		...
		v = e.parseFracTime(Time{time.Unix(int64(t), 0), 0})
	case MYSQL_TYPE_TIMESTAMP2:
		var t Time
		t, n, err = decodeTimestamp2(data, meta)
		v = e.parseFracTime(t)
	case MYSQL_TYPE_DATETIME:
		...
		d := i64 / 1000000
		t := i64 % 1000000
		v = e.parseFracTime(Time{time.Date(int(d/10000),
	...
	case MYSQL_TYPE_DATETIME2:
		var t Time
		t, n, err = decodeDatetime2(data, meta)
		v = e.parseFracTime(t)
...
func decodeTimestamp2(data []byte, dec uint16) (Time, int, error) {
	...
	if sec == 0 {
		return Time{Frac: int(dec)}, n, nil
	}

	return Time{time.Unix(sec, usec*1000), int(dec)}, n, nil
}

const DATETIMEF_INT_OFS int64 = 0x8000000000

func decodeDatetime2(data []byte, dec uint16) (Time, int, error) {
	...
	return Time{time.Date(year, time.Month(month), day, hour, minute, second, int(frac*1000), time.UTC), int(dec)}, n, nil
}
