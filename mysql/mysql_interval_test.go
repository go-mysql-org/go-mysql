...
func FuzzParseInterval(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, tag, str string) {
		_, _ = parseInterval(tag, str)
	})
}
...
func TestIntervalSlice_Contain(t *testing.T) {
	cases := []struct {
		...
		{
			IntervalSlice{
				Interval{Start: 1, Stop: 3, Tag: "foo"},
			},
			IntervalSlice{
				Interval{Start: 1, Stop: 3},
			},
		...
		},
		{
			...
			IntervalSlice{
				Interval{Start: 1, Stop: 3, Tag: "bar"},
			},
			false,
		},
		{
...
func TestIntervalSlice_Equal(t *testing.T) {
	cases := []struct {
		...
		{
			IntervalSlice{
				Interval{Start: 10, Stop: 30, Tag: "foo"},
			},
			IntervalSlice{
				Interval{Start: 10, Stop: 30, Tag: "bar"},
			},
			false,
		},
		{
			IntervalSlice{
				Interval{Start: 10, Stop: 30, Tag: "_"},
			},
			IntervalSlice{
				Interval{Start: 10, Stop: 30, Tag: "_"},
			},
...
func TestIntervalSlice_InsertInterval(t *testing.T) {
	...
	i.InsertInterval(Interval{1, 100, "abc"})
	require.Equal(t, IntervalSlice{Interval{1, 1020, ""}, Interval{1, 100, "abc"}}, i)

	i.InsertInterval(Interval{100, 150, "abc"})
	require.Equal(t, IntervalSlice{Interval{1, 1020, ""}, Interval{1, 150, "abc"}}, i)

	i.InsertInterval(Interval{1, 3, "xyz"})
	require.Equal(t, IntervalSlice{Interval{1, 1020, ""}, Interval{1, 150, "abc"}, Interval{1, 3, "xyz"}}, i)
}
...
func TestIntervalSlice_Sort(t *testing.T) {
	cases := []struct {
		...
		},
		{
			IntervalSlice{
				Interval{Start: 1, Stop: 5, Tag: "foo"},
				Interval{Start: 1, Stop: 5, Tag: "bar"},
			},
			IntervalSlice{
				Interval{Start: 1, Stop: 5, Tag: "bar"},
				Interval{Start: 1, Stop: 5, Tag: "foo"},
