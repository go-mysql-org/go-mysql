...
func TestParseUUIDSet(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			IntervalSlice{
				Interval{Start: 1, Stop: 5, Tag: "foo"},
			},
		},
		{
			...
			IntervalSlice{
				Interval{Start: 1, Stop: 5, Tag: "foo"},
				Interval{Start: 6, Stop: 9, Tag: "foo"},
			},
		},
		{
			"40eb4bee-1972-11f1-acc9-324f96ede8f2:bar:6-8:10:foo:1-4:6-8",
			IntervalSlice{
				Interval{Start: 6, Stop: 9, Tag: "bar"},
				Interval{Start: 10, Stop: 11, Tag: "bar"},
				Interval{Start: 1, Stop: 5, Tag: "foo"},
				Interval{Start: 6, Stop: 9, Tag: "foo"},
			},
		},
	...
	for _, tc := range cases {
		...
		require.Equal(t, tc.gtid, uuidset.String())
...
func TestNewUUIDSet(t *testing.T) {
	u := uuid.New()
	s := NewUUIDSet(u)
	require.Equal(t, &UUIDSet{SID: u}, s)

	s2 := NewUUIDSet(u, Interval{Start: 1, Stop: 5})
	require.Equal(t,
		&UUIDSet{
			SID: u,
			Intervals: IntervalSlice{
	...
	s3 := NewUUIDSet(u, Interval{Start: 1, Stop: 5},
		Interval{Start: 1, Stop: 10, Tag: "mytag"})
	require.Equal(t,
		&UUIDSet{
			SID: u,
			Intervals: IntervalSlice{
	...
	s4 := NewUUIDSet(u,
		Interval{Start: 10, Stop: 12, Tag: "mytag"},
		Interval{Start: 1, Stop: 5},
	...
	require.Equal(t,
		&UUIDSet{
			...
			Intervals: IntervalSlice{
				...
				Interval{Start: 1, Stop: 12, Tag: "mytag"},
			},
		},
		s4,
	)
...
func TestUUIDSet_Decode(t *testing.T) {
	u := uuid.UUID{13, 253, 150, 255, 112, 142, 65, 232, 128, 185, 191, 63, 181, 206, 227, 252}
	cases := []struct {
		format    GtidFormat
		input     []byte
		set       *UUIDSet
	...
	}{
		{
			...
			NewUUIDSet(u, Interval{Start: 1, Stop: 10}),
			false,
		},
		{
			GtidFormatClassic,
		...
		},
		{
			...
			NewUUIDSet(u, Interval{Start: 1, Stop: 10}, Interval{Start: 123, Stop: 30000000}),
			false,
		},
		{
			...
			NewUUIDSet(
				uuid.UUID{0x7, 0x1a, 0x4e, 0xcc, 0x1b, 0xf9, 0x11, 0xf1, 0xa8, 0x38, 0xe6, 0xdd, 0x18, 0x7, 0xd0, 0x29},
				Interval{Start: 1, Stop: 3, Tag: "mytagabcdef"},
			),
			false,
		},
		{
			...
			NewUUIDSet(
				...
				Interval{Start: 1, Stop: 2, Tag: "mytagabcdef"},
			),
			false,
		},
	}
...
func TestUUIDSet_MinusInterval(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			NewUUIDSet(u, Interval{Start: 1, Stop: 8}),
		},
		{
			NewUUIDSet(u, Interval{Start: 1, Stop: 10, Tag: "abc"}),
			IntervalSlice{
				Interval{Start: 8, Stop: 10, Tag: "abc"},
			},
			NewUUIDSet(u, Interval{Start: 1, Stop: 8, Tag: "abc"}),
		},
		{
			NewUUIDSet(u, Interval{Start: 1, Stop: 10}),
			IntervalSlice{
